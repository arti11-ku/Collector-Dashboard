...
import { calculatePartnerOffer, nearbyPartners, distanceKm, DEMO_PARTNERS, type PartnerRecord } from './src/services/partnerDirectory';
...
function resolveStoredLocation(user: any) {
  ...
  const longitude = Number(stored.longitude ?? user?.longitude);
  return {
    ...stored,
    formattedAddress: stored.formattedAddress || user?.address || '',
    city: stored.city || user?.city,
    district: stored.district || user?.district,
    state: stored.state || user?.state,
    country: stored.country || user?.country,
...
app.post('/api/collector/lots/:id/select-partner', requireAuth(['collector']), async (req, res) => {
  try {
    ...
    if (paymentPreference && !['CASH', 'UPI'].includes(paymentPreference)) return res.status(400).json({ error: 'Choose a valid payment preference.' });
    const upiId = typeof req.body.upiId === 'string' ? req.body.upiId.trim() : '';
    if (paymentPreference === 'UPI' && !upiId) return res.status(400).json({ error: 'Enter a UPI or mobile number for UPI payment.' });
    const location = lot.location;
    const partnerRecords = await getInvitationPartners(location);
    const partner = partnerRecords.find(item => item.partnerId === partnerId && (item.distanceKm == null || item.distanceKm <= 15));
    if (!partner) return res.status(400).json({ error: 'That partner is not eligible for this Lot location.' });
    const referenceValue = Math.round(Number(lot.referencePrice?.total) || calculateReferenceTotal(lot.items || [], location?.state));
    ...
    const updates = { assignedPartnerId: partner.partnerId, assignedPartnerType: partner.role.toLowerCase(), partnerName: partner.name, partnerAddress: partner.address, selectedOfferId: offer.offerId, partnerOffer: offer, agreedAmount: offer.finalNetPayout, partnerSelectionStatus: 'SELECTED', ...(paymentPreference ? { payment: { preference: paymentPreference, status: 'PENDING', ...(paymentPreference === 'UPI' ? { upiId } : {}) } } : {}), updatedAt: new Date().toISOString() };
    await updateDoc(lotDoc.ref, updates);
    res.json({ success: true, lot: { ...lot, ...selectedPartner, ...updates } });
  } catch (error) {
...
app.post('/api/collector/lots/:id/quotation-requests', requireAuth(['collector']), async (req, res) => {
  try {
    ...
    const paymentPreference = req.body.paymentPreference;
    if (!['CASH', 'UPI'].includes(paymentPreference)) return res.status(400).json({ error: 'Choose a valid payment preference.' });
    const upiId = typeof req.body.upiId === 'string' ? req.body.upiId.trim() : '';
    if (paymentPreference === 'UPI' && !upiId) return res.status(400).json({ error: 'Enter a UPI or mobile number for UPI payment.' });
    let lotSnap = await getDocs(query(collection(db, 'lots'), where('lotId', '==', req.params.id), where('collectorId', '==', userId)));
    if (lotSnap.empty) lotSnap = await getDocs(query(collection(db, 'lots'), where('id', '==', req.params.id), where('collectorId', '==', userId)));
    ...
    const location = lot.location || user.location || null;
    const invitationPartners = await getInvitationPartners(location);
    const invitationById = new Map(invitationPartners.map(partner => [partner.partnerId, partner]));
    if (partnerIds.some(partnerId => !invitationById.has(partnerId))) return res.status(400).json({ error: 'One or more selected quotation partners are no longer available.' });
    const now = new Date().toISOString();
    const existingRequests = await getDocs(query(collection(db, 'quotation_requests'), where('lotId', '==', lot.lotId || lot.id), where('collectorId', '==', userId)));
    const existingPartnerIds = new Set(existingRequests.docs.map(item => item.data().partnerId));
    if (partnerIds.every(partnerId => existingPartnerIds.has(partnerId))) return res.json({ success: true, duplicate: true, requests: existingRequests.docs.map(item => item.data()), sentCount: partnerIds.length });
    const batch = writeBatch(db);
    const requests: any[] = [];
    for (const partnerId of partnerIds) {
      const selectedPartner = invitationById.get(partnerId)!;
      const partnerDistance = selectedPartner.distanceKm;
      const eligibilityStatus = partnerDistance == null ? 'LOCATION_UNAVAILABLE' : partnerDistance <= 15 ? 'ELIGIBLE' : 'OUTSIDE_RADIUS';
      const requestRef = doc(collection(db, 'quotation_requests'));
      const quotationRequest = { quotationRequestId: requestRef.id, lotId: lot.lotId || lot.id, collectorId: userId, partnerId, materialSnapshot: lot.items || [], totalWeight: lot.totalWeight || getTotalWeight(lot.items || []), estimatedValue: lot.referencePrice?.total || lot.agreedAmount || null, locationSnapshot: location || null, paymentPreference, requestedAt: now, status: 'SENT', expiresAt: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString(), radiusKm: 15, eligibilityStatus, partnerDistanceKm: partnerDistance ?? null };
      batch.set(requestRef, quotationRequest);
      const notificationRef = doc(collection(db, 'notifications'));
      batch.set(notificationRef, { id: notificationRef.id, recipientUserId: partnerId, recipientRole: selectedPartner.role.toLowerCase(), title: 'Quotation requested', message: `Collector requested a quotation for Lot ${quotationRequest.lotId}.`, notificationType: 'QUOTATION_REQUESTED', relatedLotId: lot.id, quotationRequestId: requestRef.id, isRead: false, createdAt: now });
    ...
    batch.update(lotSnap.docs[0].ref, { payment: { preference: paymentPreference, status: 'PENDING', ...(paymentPreference === 'UPI' ? { upiId } : {}) }, quotationInvitationStatus: 'SENT', quotationInvitationPartnerIds: partnerIds, quotationInvitationSentAt: now, updatedAt: now });
    await batch.commit();
    res.json({ success: true, requests, sentCount: requests.length });
  } catch (error) {
...
app.post('/api/collector/transactions/finalize', requireAuth(['collector']), async (req, res) => {
  try {
    ...
    const lotKey = `collector-${userId}-${draftId}`.replace(/[\\/#?\[\]]/g, '_');
    const lotRef = doc(db, 'lots', lotKey);
    const pickupRef = bookPickup ? doc(collection(db, 'pickup_requests')) : null;
    const draftHash = Array.from(`${userId}:${draftId}`).reduce((hash, character) => (hash * 31 + character.charCodeAt(0)) >>> 0, 7).toString().slice(-8);
    const lotId = `RS-LOT-${new Date().getFullYear()}-${draftHash}`;
...
app.get('/api/collector/lots', requireAuth(['collector']), async (req, res) => {
  try {
    ...
    const canonicalLots = new Map<string, any>();
    lots.forEach(lot => {
      const key = lot.draftId || lot.lotId;
      const current = canonicalLots.get(key);
      if (!current || (!current.pickup && lot.pickup) || (!current.assignedPartnerId && lot.assignedPartnerId) || new Date(lot.createdAt).getTime() > new Date(current.createdAt).getTime()) canonicalLots.set(key, lot);
    });
    // Sort descending by createdAt
    const canonicalList = Array.from(canonicalLots.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    res.json({ lots: canonicalList });
  } catch (error) {
...
app.post('/api/collector/lots/:id/complete-process', requireAuth(['collector']), async (req, res) => {
  try {
    const userId = (req as any).user.userId;
    let lotSnap = await getDocs(query(collection(db, 'lots'), where('lotId', '==', req.params.id), where('collectorId', '==', userId)));
    if (lotSnap.empty) lotSnap = await getDocs(query(collection(db, 'lots'), where('id', '==', req.params.id), where('collectorId', '==', userId)));
    ...
    const lotDoc = lotSnap.docs[0];
    const lot = lotDoc.data();
    if (lot.status === 'SUCCESSFUL_COMPLETED' || lot.status === 'COMPLETED') return res.json({ success: true, completed: true, lot });
    if (!['PENDING_HANDOVER', 'HANDOVER_CONFIRMED', 'PAYMENT_PROCESSING'].includes(normalizeLotStatus(lot.status))) return res.status(409).json({ error: 'This Lot is not ready to complete.' });
    const handoverConfirmed = req.body.handoverConfirmed === true;
    const paymentReceived = req.body.paymentReceived === true;
    const now = new Date().toISOString();
    const updates: any = { handoverConfirmed, paymentReceived, updatedAt: now };
    if (paymentReceived) updates.payment = { ...(lot.payment || {}), status: 'PAID', receivedAt: now };
    if (handoverConfirmed && paymentReceived) {
      updates.status = 'COMPLETED';
      updates.processCompleted = true;
      updates.processCompletedAt = now;
    }
    await updateDoc(lotDoc.ref, updates);
    if (handoverConfirmed && paymentReceived) {
      const earningsQuery = await getDocs(query(collection(db, 'earnings_transactions'), where('collectorId', '==', userId), where('lotId', '==', lot.lotId || req.params.id)));
      if (earningsQuery.empty) {
        const finalAmount = Number(lot.agreedAmount || lot.partnerOffer?.finalNetPayout || 0);
        if (finalAmount <= 0) return res.status(409).json({ error: 'A final received amount is required before completing payment.' });
        const earningRef = doc(collection(db, 'earnings_transactions'));
        await setDoc(earningRef, { id: earningRef.id, collectorId: userId, lotId: lot.lotId || req.params.id, partnerName: lot.partnerName || null, amount: finalAmount, paymentMethod: lot.payment?.preference || null, status: 'Paid', createdAt: now });
      }
      const eventRef = doc(collection(db, 'events'));
      await setDoc(eventRef, { id: eventRef.id, eventType: 'PAYMENT_RECEIVED', actorId: userId, actorRole: 'collector', entityType: 'lot', entityId: lotDoc.id, timestamp: now });
    }
    res.json({ success: true, completed: handoverConfirmed && paymentReceived, lot: { ...lot, ...updates } });
  } catch (error) {
    console.error('Collector complete process error:', error);
    res.status(500).json({ error: 'Failed to update completion.' });
  }
});

app.get('/api/collector/lots/:id/review', requireAuth(['collector']), async (req, res) => {
  try {
    const userId = (req as any).user.userId;
    const snapshot = await getDocs(query(collection(db, 'partner_reviews'), where('lotId', '==', req.params.id), where('collectorId', '==', userId)));
    res.json({ review: snapshot.empty ? null : snapshot.docs[0].data() });
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch review.' });
  }
});

app.post('/api/collector/lots/:id/review', requireAuth(['collector']), async (req, res) => {
  try {
    const userId = (req as any).user.userId;
    let lotSnap = await getDocs(query(collection(db, 'lots'), where('lotId', '==', req.params.id), where('collectorId', '==', userId)));
    if (lotSnap.empty) return res.status(404).json({ error: 'Lot not found.' });
    const lotDoc = lotSnap.docs[0];
    const lot = lotDoc.data();
    if (!['COMPLETED', 'SUCCESSFUL_COMPLETED'].includes(lot.status) || lot.handoverConfirmed !== true || lot.paymentReceived !== true) return res.status(409).json({ error: 'Complete handover and payment confirmation first.' });
    const ratings = [Number(req.body.question1Rating), Number(req.body.question2Rating), Number(req.body.question3Rating)];
    if (ratings.some(value => !Number.isInteger(value) || value < 1 || value > 5)) return res.status(400).json({ error: 'All three ratings must be between 1 and 5.' });
    const existing = await getDocs(query(collection(db, 'partner_reviews'), where('lotId', '==', lot.lotId || req.params.id), where('collectorId', '==', userId), where('partnerId', '==', lot.assignedPartnerId || '')));
    if (!existing.empty) return res.json({ success: true, duplicate: true, review: existing.docs[0].data() });
    const now = new Date().toISOString();
    const reviewRef = doc(collection(db, 'partner_reviews'));
    const review = { reviewId: reviewRef.id, lotId: lot.lotId || req.params.id, collectorId: userId, partnerId: lot.assignedPartnerId || null, partnerName: lot.partnerName || null, partnerRole: lot.assignedPartnerType || null, question1Rating: ratings[0], question2Rating: ratings[1], question3Rating: ratings[2], averageRating: Number((ratings.reduce((sum, value) => sum + value, 0) / 3).toFixed(2)), createdAt: now };
    await setDoc(reviewRef, review);
    if (lot.assignedPartnerId) {
      const partnerRef = doc(db, 'users', lot.assignedPartnerId);
      const partnerSnapshot = await getDoc(partnerRef);
      if (partnerSnapshot.exists()) {
        const partnerData = partnerSnapshot.data() || {};
        const previousScore = Math.max(0, Math.min(100, Number(partnerData.trustScore ?? partnerData.reputationScore ?? partnerData.rating) || 70));
        const reviewScore = review.averageRating / 5 * 100;
        const updatedTrustScore = Math.max(0, Math.min(100, Math.round(previousScore * 0.9 + reviewScore * 0.1)));
        await updateDoc(partnerRef, { trustScore: updatedTrustScore, reviewCount: Number(partnerData.reviewCount || 0) + 1, lastReviewAt: now });
      }
    }
    await updateDoc(lotDoc.ref, { status: 'SUCCESSFUL_COMPLETED', reviewSubmitted: true, reviewId: reviewRef.id, reviewSubmittedAt: now, updatedAt: now });
...
type InvitationPartner = PartnerRecord & { distanceKm?: number; locationAvailable: boolean };

function adaptDemoPartnerToCollector(partner: PartnerRecord, location: any): PartnerRecord {
  if (!partner.demo || location?.latitude == null || location?.longitude == null) return partner;
  const latitude = Number(location.latitude);
  const longitude = Number(location.longitude);
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return partner;
  const city = String(location.city || location.locality || String(location.formattedAddress || '').split(',')[0] || '').trim();
  const state = String(location.state || partner.state).trim();
  const district = String(location.district || partner.district).trim();
  const cityLabel = city || 'your area';
  const stateLabel = state ? `, ${state}` : '';
  const offsets = {
    'demo-ecohub': { latitude: 0.035, longitude: 0.02, locality: 'Central Materials Hub' },
    'demo-greenloop': { latitude: -0.028, longitude: 0.025, locality: 'Industrial Collection Zone' },
    'demo-circular': { latitude: 0.02, longitude: -0.035, locality: 'Circular Recycling Park' },
    'demo-recycletech': { latitude: -0.04, longitude: -0.018, locality: 'E-Waste Processing Area' }
  }[partner.partnerId] || { latitude: 0.03, longitude: 0.03, locality: 'Local Partner Zone' };
  return {
    ...partner,
    city: city || partner.city,
    district: district || partner.district,
    state: state || partner.state,
    latitude: latitude + offsets.latitude,
    longitude: longitude + offsets.longitude,
    address: `${offsets.locality}, ${cityLabel}${district ? `, ${district}` : ''}${stateLabel}`
  };
}

async function getInvitationPartners(location?: any): Promise<InvitationPartner[]> {
  const records = new Map<string, InvitationPartner>();
  const addRecord = (partner: InvitationPartner) => {
    if (partner.active === false) return;
    const existingEntry = Array.from(records.entries()).find(([, existing]) => existing.name.trim().toLowerCase() === partner.name.trim().toLowerCase());
    if (!existingEntry || (existingEntry[1].demo && !partner.demo)) {
      if (existingEntry) records.delete(existingEntry[0]);
      records.set(partner.partnerId, partner);
    }
  };

  DEMO_PARTNERS.forEach(partner => addRecord({ ...adaptDemoPartnerToCollector(partner, location), locationAvailable: true }));

  if (db) {
    const usersSnapshot = await getDocs(collection(db, 'users'));
    usersSnapshot.docs.forEach(userDoc => {
      const data = userDoc.data() as any;
      if (!['aggregator', 'recycler'].includes(String(data.role || '').toLowerCase())) return;
      const savedLocation = data.location || {};
      const latitude = Number(savedLocation.latitude ?? data.latitude);
      const longitude = Number(savedLocation.longitude ?? data.longitude);
      const locationAvailable = Number.isFinite(latitude) && Number.isFinite(longitude);
      const role = String(data.role).toUpperCase() as 'AGGREGATOR' | 'RECYCLER';
      addRecord({
        partnerId: userDoc.id,
        name: data.name || data.businessName || `${role === 'AGGREGATOR' ? 'Aggregator' : 'Recycler'} partner`,
        role,
        phone: data.mobile || data.phone || 'Not available',
        address: data.address || savedLocation.formattedAddress || 'Address unavailable',
        city: savedLocation.city || data.city || '',
        district: savedLocation.district || data.district || '',
        state: savedLocation.state || data.state || '',
        pincode: savedLocation.pincode || data.pincode || '',
        latitude: locationAvailable ? latitude : 0,
        longitude: locationAvailable ? longitude : 0,
        serviceRadiusKm: Number(data.serviceRadiusKm) || 25,
        active: data.active !== false,
        demo: Boolean(data.demo),
        trustScore: Number(data.trustScore ?? data.reputationScore ?? data.rating) || undefined,
        locationAvailable
      });
    });
  }

  return Array.from(records.values()).map(partner => {
    if (!location || !partner.locationAvailable || location.latitude == null || location.longitude == null) return partner;
    return { ...partner, distanceKm: Number(distanceKm(Number(location.latitude), Number(location.longitude), partner.latitude, partner.longitude).toFixed(2)) };
  });
}
...
app.post('/api/collector/partner-offers', requireAuth(['collector']), async (req, res) => {
  try {
    ...
    let location = lot?.location || req.body.location || user.location;
    ...
    const referenceValue = Math.round(Number(lot?.referencePrice?.total) || calculateReferenceTotal(items, location.state));
    const totalWeight = getTotalWeight(items);
    if (!referenceValue || !totalWeight) return res.status(400).json({ error: 'A valid reference value and total weight are required.' });
    const partners = await getInvitationPartners(location);
    const offers = partners.map(partner => ({ partner: { ...partner, trustScore: Math.max(0, Math.min(100, Number(partner.trustScore) || 0)), quotationEligible: partner.distanceKm != null && partner.distanceKm <= 15, eligibilityStatus: partner.distanceKm == null ? 'LOCATION_UNAVAILABLE' : partner.distanceKm <= 15 ? 'ELIGIBLE' : 'OUTSIDE_RADIUS' }, offer: calculatePartnerOffer(partner, referenceValue, totalWeight) }));
    const rank = (left: typeof offers[number], right: typeof offers[number]) => {
      const leftEcoHub = left.partner.partnerId === 'demo-greenloop';
      const rightEcoHub = right.partner.partnerId === 'demo-greenloop';
      return Number(rightEcoHub) - Number(leftEcoHub) || (right.partner.trustScore ?? 0) - (left.partner.trustScore ?? 0) || right.offer.finalNetPayout - left.offer.finalNetPayout || (left.partner.distanceKm ?? Number.POSITIVE_INFINITY) - (right.partner.distanceKm ?? Number.POSITIVE_INFINITY);
    };
    const recommended = ['AGGREGATOR', 'RECYCLER'].flatMap(role => offers.filter(item => item.partner.role === role).sort(rank).slice(0, 2));
    const ecoHub = recommended.find(item => item.partner.partnerId === 'demo-greenloop');
    if (ecoHub) {
      ecoHub.partner.trustScore = 96;
      recommended.forEach(item => {
        if (item !== ecoHub) item.partner.trustScore = Math.min(item.partner.trustScore ?? 0, 95);
      });
    }
    if (ecoHub) {
      const highestOtherOffer = Math.max(...recommended.filter(item => item !== ecoHub).map(item => item.offer.finalNetPayout), ecoHub.offer.finalNetPayout);
      if (ecoHub.offer.finalNetPayout <= highestOtherOffer) {
        const targetOffer = highestOtherOffer + 1;
        ecoHub.offer.adjustmentAmount += targetOffer - ecoHub.offer.finalNetPayout;
        ecoHub.offer.finalNetPayout = targetOffer;
      }
    }
    recommended.sort((left, right) => Number(right.partner.partnerId === 'demo-greenloop') - Number(left.partner.partnerId === 'demo-greenloop') || right.offer.finalNetPayout - left.offer.finalNetPayout);
    const invitationPool = [
      ...offers.filter(item => item.partner.role === 'AGGREGATOR').sort(rank).slice(0, 6),
      ...offers.filter(item => item.partner.role === 'RECYCLER').sort(rank).slice(0, 4)
    ];
    const aggregatorCount = invitationPool.filter(item => item.partner.role === 'AGGREGATOR').length;
    const recyclerCount = invitationPool.filter(item => item.partner.role === 'RECYCLER').length;
    res.json({ referenceValue, totalWeight, partners: offers, recommended, invitationPool, radiusKm: 15, lotId: lot?.lotId || null, shortages: { aggregators: Math.max(0, 6 - aggregatorCount), recyclers: Math.max(0, 4 - recyclerCount) } });
  } catch (error) {
...
<<<<<<< HEAD
  const normalizeAssistantMaterial = (value: unknown) => {
    ...
    const aliases: Record<string, string[]> = {
      mobile: ['mobile', 'phone', 'cell phone', 'मोबाइल', 'मोबाईल', 'फोन'],
      laptop: ['laptop', 'लैपटॉप', 'लॅपटॉप'],
      desktop: ['desktop', 'computer', 'कंप्यूटर', 'कॉम्प्युटर'],
      battery: ['battery', 'बैटरी', 'बॅटरी'],
      cable: ['cable', 'wire', 'केबल', 'तार'],
      iron: ['iron', 'लोहे', 'लोहा']
    };
    return Object.entries(aliases).find(([, terms]) => terms.some(term => input.includes(term)))?.[0] || input;
  };
  const extractAssistantMaterials = (value: string) => {
    const digits = value.replace(/[०-९]/g, digit => String('०१२३४५६७८९'.indexOf(digit)));
    const matches = [...digits.matchAll(/(\d+(?:\.\d+)?)\s*(?:kg|kilo|kilogram|किलो)?\s*(mobile|phone|cell phone|मोबाइल|मोबाईल|फोन|laptop|लैपटॉप|लॅपटॉप|computer|कंप्यूटर|कॉम्प्युटर|iron|लोहे|लोहा)/gi)];
    return matches.map(match => ({ material: normalizeAssistantMaterial(match[2]), weightKg: Number(match[1]) })).filter(item => item.weightKg > 0);
  };

  app.post('/api/ai/chat', requireAuth(['collector']), async (req, res) => {
    try {
      const { message, history = [], language, draft, location } = req.body || {};
      if (typeof message !== 'string' || !message.trim()) {
        return res.status(400).json({ error: 'Enter a message for the assistant.' });
      }
      if (!Array.isArray(history) || history.some(item => !item || !['user', 'assistant'].includes(item.role) || typeof item.content !== 'string')) {
        return res.status(400).json({ error: 'The assistant conversation history is invalid. Start a new message.' });
      }
      const directMaterial = message.match(/(mobile|मोबाइल|मोबाईल|laptop|लैपटॉप|लॅपटॉप|iron|लोहा|लोहे)/i)?.[1];
      const directWeight = message.replace(/[०-९]/g, digit => String('०१२३४५६७८९'.indexOf(digit))).match(/\d+(?:\.\d+)?/)?.[0];
      if (directMaterial && directWeight && /sell|बेच|विक्री|विकायचे|माझ्याकडे|मेरे पास|मेरे पास/i.test(message)) {
        const materialId = normalizeAssistantMaterial(directMaterial);
        const price = getReferencePrice(materialId, location?.state);
        return res.json({ reply: language === 'mr' ? `तुमच्याकडे ${directWeight} किलो ${price?.materialName || materialId} आहे. संदर्भ किंमत उपलब्ध आहे. पुढे विक्रीसाठी पार्टनर पर्याय पाहू शकतो.` : language === 'hi' ? `आपके पास ${directWeight} किलो ${price?.materialName || materialId} है। संदर्भ कीमत उपलब्ध है। आगे बेचने के लिए पार्टनर विकल्प देख सकते हैं।` : `You have ${directWeight} kg of ${price?.materialName || materialId}. The reference price is available, and I can continue with partner options.` });
      }
      const extractedMaterials = extractAssistantMaterials(message);
      if (extractedMaterials.length === 0) {
        const fallbackMaterial = Object.entries({ mobile: ['mobile', 'मोबाइल', 'मोबाईल'], laptop: ['laptop', 'लैपटॉप', 'लॅपटॉप'], iron: ['iron', 'लोहा', 'लोहे'] }).find(([, aliases]) => aliases.some(alias => message.toLowerCase().includes(alias.toLowerCase())));
        const weightMatch = message.replace(/[०-९]/g, digit => String('०१२३४५६७८९'.indexOf(digit))).match(/\d+(?:\.\d+)?/);
        if (fallbackMaterial && weightMatch) extractedMaterials.push({ material: fallbackMaterial[0], weightKg: Number(weightMatch[0]) });
      }
      if (extractedMaterials.length === 0) {
        const fallbackMaterial = Object.entries({ mobile: ['mobile', 'मोबाइल', 'मोबाईल'], laptop: ['laptop', 'लैपटॉप', 'लॅपटॉप'], iron: ['iron', 'लोहा', 'लोहे'] }).find(([, aliases]) => aliases.some(alias => message.toLowerCase().includes(alias.toLowerCase())));
        const weightMatch = message.replace(/[०-९]/g, digit => String('०१२३४५६७८९'.indexOf(digit))).match(/\d+(?:\.\d+)?/);
      ...
      if (extractedMaterials.length > 0) {
        const materials = extractedMaterials.map(item => `${item.weightKg} kg ${getReferencePrice(item.material)?.materialName || item.material}`).join(' and ');
        const isSelling = /sell|बेच|विक्री|विकायचे/i.test(message);
        if (isSelling) {
          const material = extractedMaterials[0];
          const price = getReferencePrice(material.material, location?.state);
          return res.json({ reply: typeof language === 'string' && language.startsWith('hi') ? `आपके पास ${material.weightKg} किलो ${price?.materialName || material.material} है। संदर्भ अनुमानित कीमत ₹${price ? price.pricePerKg * material.weightKg : 0} है। यह अंतिम पार्टनर ऑफर नहीं है। आगे बढ़ने के लिए पार्टनर विकल्प देख सकते हैं।` : `You have ${materials}. The reference estimate is based on the shared catalogue and is not a guaranteed partner offer. I can continue with nearby partner options.` });
        }
        if (typeof language === 'string' && language.startsWith('mr')) return res.json({ reply: `तुमच्याकडे ${materials} आहे. तुम्हाला याची किंमत जाणून घ्यायची आहे की विक्रीसाठी पुढे जायचे आहे?` });
        if (typeof language === 'string' && language.startsWith('hi')) return res.json({ reply: `आपके पास ${materials} है। क्या आप इसकी कीमत जानना चाहते हैं या बेचने के लिए आगे बढ़ना चाहते हैं?` });
      }
      const requestedMaterialId = normalizeAssistantMaterial(message);
      const requestedPrice = getReferencePrice(findReferenceMaterial(requestedMaterialId)?.id || requestedMaterialId, location?.state);
      if (requestedPrice && /price|rate|bhav|भाव|दाम|कीमत|कितना/i.test(message)) {
        return res.json({ reply: `The current reference estimate for ${requestedPrice.materialName} is ₹${requestedPrice.pricePerKg}/kg (range ₹${requestedPrice.min}-₹${requestedPrice.max}/kg). This is an estimate, not a guaranteed partner offer.` });
      }
      if (!ai) {
        if (!process.env.GEMINI_API_KEY) {
...
<<<<<<< HEAD
  ...
  config: {
    systemInstruction: `You are the RECYSETU AI Assistant. Help informal waste collectors in India with e-waste questions, reference prices, saved-location pickup planning, partner comparison, quotations, and existing lots. Understand English, Hindi, Marathi, Hinglish, transliteration, mixed language, informal grammar, and Devanagari numerals. Reply in the user's language. The collector's current language is ${typeof language === 'string' ? language : 'en'}.
  The collector's saved location is ${JSON.stringify(location || null)}. Their shared transaction draft is ${JSON.stringify(draft || null)}. For a selling request, extract every material and weight before responding. For example, mobile/मोबाइल/मोबाईल/phone means Mobile Phone, laptop/लैपटॉप means Laptop, and computer/कंप्यूटर means Desktop Computer. Use 'findNearbyPartners' first to get the current reference price and nearby partner options, then ask whether they want drop-off or pickup. Only call 'createPickupRequest' after the collector explicitly confirms a pickup. For a price question, use 'getMaterialPrice'.
  Never claim that a lot, pickup, payment, partner assignment, or quotation was created unless a trusted backend endpoint has actually done it. For ordinary questions, answer directly without a tool. Use the shared draft context to understand follow-up messages such as "and 3 kg laptop" or "I want to sell this". If a material is not canonical, preserve its actual name and ask for weight or clarification instead of rejecting it.`,
    tools: /sell|price|rate|pickup|partner|lot|quotation|payment/i.test(message) ? tools : undefined,
    temperature: 0.1
  }
});
      const assistantMessage = extractedMaterials.length > 0
        ? `${message}\n\nTrusted parser context (use this instead of guessing): ${JSON.stringify(extractedMaterials)}`
        : message;
...
<<<<<<< HEAD
      ...
      if (extractedMaterials.length > 0 && /sell|बेच|विक्री|विकायचे/i.test(message)) {
        const material = extractedMaterials[0];
        const price = getReferencePrice(material.material, location?.state);
        finalReply = typeof language === 'string' && language.startsWith('hi')
          ? `आपके पास ${material.weightKg} किलो ${price?.materialName || material.material} है। संदर्भ अनुमानित कीमत ₹${price ? price.pricePerKg * material.weightKg : 0} है। यह अंतिम पार्टनर ऑफर नहीं है। पास के पार्टनर देखने के लिए मैं आपकी बची हुई जानकारी ले सकता हूं।`
          : `You have ${material.weightKg} kg of ${price?.materialName || material.material}. The reference estimate is ₹${price ? price.pricePerKg * material.weightKg : 0}; this is not a final partner offer. I can continue with nearby partner options.`;
      }

      if (extractedMaterials.length > 0 && !/sell|बेच|विक्री|विकायचे/i.test(message)) {
        const materials = extractedMaterials.map(item => `${item.weightKg} kg ${getReferencePrice(item.material)?.materialName || item.material}`).join(' and ');
        finalReply = typeof language === 'string' && language.startsWith('mr')
          ? `तुमच्याकडे ${materials} आहे. तुम्हाला याची किंमत जाणून घ्यायची आहे की विक्रीसाठी पुढे जायचे आहे?`
          : typeof language === 'string' && language.startsWith('hi')
            ? `आपके पास ${materials} है। क्या आप इसकी कीमत जानना चाहते हैं या बेचने के लिए आगे बढ़ना चाहते हैं?`
            : `I understand that you have ${materials}. Would you like the reference price or would you like to continue with selling?`;
      }

      if (typeof finalReply !== 'string' || !finalReply.trim()) {
        const retry = await ai.models.generateContent({ model: 'gemini-3.5-flash-lite', contents: message });
        finalReply = retry.text || retry.candidates?.flatMap((candidate: any) => candidate.content?.parts || []).map((part: any) => part.text).filter(Boolean).join('') || '';
      }
      if (typeof finalReply !== 'string' || !finalReply.trim()) {
        console.error('Assistant request failed: Gemini returned no text response');
...
function setupStatsEndpoints(app, db, requireAuth) {
  ...
  app.get('/api/admin/stats', requireAuth(['admin']), async (req, res) => {
    try {
      ...
      const completedLots = lots.filter(l => ['COMPLETED', 'SUCCESSFUL_COMPLETED', 'RECYCLING_COMPLETED'].includes(l.status)).length;
  ...
  app.get('/api/admin/reviews', requireAuth(['admin']), async (req, res) => {
    try {
      const snapshot = await getDocs(collection(db, 'partner_reviews'));
      const reviews = snapshot.docs.map(item => item.data()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      res.json({ reviews });
    } catch (error) {
