  ...
  return (
    <div className="space-y-6 pb-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white border border-[#E0E7E1] rounded-[32px] p-6 md:p-8 shadow-sm space-y-7">
        ...
        <div className="grid md:grid-cols-2 gap-3"><button type="button" disabled={items.length === 0 || items.some(item => !item.estimatedRate)} onClick={() => navigate('/collector/check-price')} className="w-full bg-[#1D3124] text-white font-bold py-4 rounded-xl disabled:opacity-50">Sell</button><button type="button" disabled={items.length === 0} onClick={() => navigate('/collector/check-price')} className="w-full border border-[#2D5A27] text-[#2D5A27] font-bold py-4 rounded-xl disabled:opacity-50">Create Lot for Later</button></div>
      </div>
    </div>
  );
}
